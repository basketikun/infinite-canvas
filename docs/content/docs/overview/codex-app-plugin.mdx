...
The plugin connects Codex to a local Canvas Agent, allowing Codex to open CoResearch, read and create nodes, connect nodes, and start generation workflows. Its source is in `plugins/infinite-canvas/`.
...
In a new Codex conversation, ask it to open CoResearch, create a canvas, and connect Codex. The plugin starts Canvas Agent, reads its connection URL and token, opens `https://canvas.best/`, and creates and connects a canvas. It starts a local frontend only when explicitly requested.
