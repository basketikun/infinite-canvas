...
| `imageModelCapabilities` | object | 图片模型参数限制配置，按模型名保存 `sizes`、`resolutions`、`aspectRatios` |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `sizes` | string[] | 允许的图片 `size` 参数列表 |
| `resolutions` | string[] | 允许的 Grok 图片分辨率列表 |
| `aspectRatios` | string[] | 允许的 Grok 图片画幅比例列表 |
