...
| `membership_level` | string | 当前会员等级：空 / `free` / `vip` / `svip` |
| `membership_expires_at` | string | 会员到期时间，RFC3339 |
...
| `membership_grant` | 购买会员套餐时赠送 |

### membership_plans

会员套餐表。后台维护可购买套餐与权益配置。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | string | 主键 |
| `name` | string | 套餐名称 |
| `level` | string | 会员等级：`vip`、`svip` |
| `description` | string | 卖点描述 |
| `price` | number | 价格，单位：分 |
| `duration_days` | number | 会员有效天数 |
| `credits_granted` | number | 购买时赠送算力点 |
| `unlimited` | bool | 会员期内是否不消耗算力点 |
| `priority_queue` | bool | 是否优先队列 |
| `features` | text | 解锁的功能 JSON 文本 |
| `enabled` | bool | 是否上架 |
| `sort` | number | 排序，升序 |
| `created_at` | string | 创建时间 |
| `updated_at` | string | 更新时间 |
...
| `plan_name` | string | 下单时套餐名称快照 |
| `plan_level` | string | 下单时套餐等级快照 |
| `amount` | number | 金额，单位：分 |
| `status` | string | 订单状态：`pending`、`paid`、`cancelled` |
| `payment_provider` | string | 支付方式：`wechat`、`alipay`、`mock` |
| `payment_id` | string | 支付方返回的凭证号 |
| `pay_url` | string | 跳转支付的地址 |
| `paid_at` | string | 支付完成时间 |
| `expires_at` | string | 本单生效后的会员到期时间 |
| `created_at` | string | 创建时间 |
| `updated_at` | string | 更新时间 |
