...
```json
{
  "modelChannel": {
    ...
    "imageModelCapabilities": {
      "gpt-image-2-4K": { "sizes": ["4k", "3840x2160", "1024x1024", "1024x1536", "1536x1024", "auto"] },
      "grok-imagine-image": { "resolutions": ["1k", "2k"], "aspectRatios": ["1:1", "16:9", "9:16"] }
    },
...
| `imageModelCapabilities` | object | 可选的图片模型参数限制，按模型名配置 `sizes`、`resolutions`、`aspectRatios`；前端会据此禁用不支持的尺寸、Grok 分辨率和画幅比例 |
...
`imageModelCapabilities` 每个模型的可选字段：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `sizes` | string[] | 该图片模型允许的 `size` 参数，例如 `1024x1024`、`1536x1024`、`4k`、`auto`；留空表示不额外限制 |
| `resolutions` | string[] | `grok-imagine-image` 可用分辨率，例如 `1k`、`2k`；留空表示不额外限制 |
| `aspectRatios` | string[] | `grok-imagine-image` 可用画幅比例，例如 `1:1`、`16:9`、`9:16`；留空表示不额外限制 |
