...
- 画布文本节点内容框输入 `@` 后，鼠标点击菜单里的缩略图、编号文字或标题摘要都应插入对应资源编号，点击菜单空白处不应触发画布拖拽或提前退出编辑态，键盘上下选择和回车插入仍保持可用。
- 画布资源节点会按当前上下文显示 `图片1/视频1/音频1/文本1` 角标，画布普通节点 prompt 文本框和配置节点文本编辑框输入 `@` 时可选择已连接资源并插入对应编号；需要验证配置节点调整输入顺序后，角标、`@` 列表和实际请求编号保持一致。
- 画布文本节点连接到生成配置节点后，文本节点内容框和 prompt 面板输入 `@` 会读取该生成配置节点已连接的参考图片/视频/音频；输入预览弹窗内编辑文本提示词时，`@` 菜单应限制在弹窗内并排除当前正在编辑的文本节点。
- 画布 `@` 菜单点击缩略图或文字应能直接插入 `图片1/视频1/音频1`，不会被画布拖拽或文本节点外部点击监听提前关闭；生成配置节点的输入统计 chip 区域应恢复为可拖动节点区域，预览、模型、参数和开始生成按钮仍不触发拖拽。
- Seedance 视频生成需同时验证标准方舟 `/api/v3` + `doubao-seedance-2-0-260128` 和 Agent Plan `/api/plan/v3` + `doubao-seedance-2.0` 两种配置都会走 `/contents/generations/tasks` JSON 异步任务接口，非 Seedance 视频模型仍走原 `/videos` FormData 流程。
