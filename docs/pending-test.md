...
- 新增公告发布功能：后台可配置公告标题、内容、展示期限、顺序和启用状态；首页会对登录和未登录用户弹出居中的公告蒙版，支持 Markdown、HTML、纯文本以及 `\n`、`<br>`、`</br>` 换行内容；公告弹窗在移动端竖屏下压缩标题和按钮区域，保留更多内容阅读空间，用户可逐条确认或选择今日不再弹出。
- 提示词库封面和预览中的远程图片链接都改为后端 `/api/prompt-images/*` 地址，后端首次访问远程图片时动态写入本地缓存，后续直接从服务器缓存返回；前台、后台和加入“我的素材”的提示词封面都会使用服务器地址。
