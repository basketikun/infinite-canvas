    ...
    const pollGenerationLog = async (log: GenerationLog, configOverride?: AiConfig) => {
        ...
        try {
            for (let attempt = 0; attempt < 120; attempt += 1) {
                ...
                await delay(log.task.provider === "seedance" || log.task.provider === "apimart" ? 5000 : 2500);
            }
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : "生成失败";
                    ...
                    <div className="thin-scrollbar flex flex-col rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto">
                        ...
                        <div className="mt-6 space-y-5">
                            <div>
                                ...
                                <Input.TextArea data-testid="video-prompt-input" value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={7} placeholder="描述镜头运动、主体动作、场景氛围和画面风格" />
                            </div>
                        ...
                        <div className="mt-auto pt-6">
                            <Button data-testid="video-generate-button" type="primary" size="large" block icon={<Sparkles className="size-4" />} loading={running} disabled={!canGenerate || running} onClick={() => void generate()}>
                                开始生成
                            </Button>
                        </div>
                    </div>

                    <div data-testid="video-results" className="thin-scrollbar rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto lg:p-5">
    ...
    return (
        <>
            <label className="col-span-2 block min-w-0 sm:col-span-1">
                ...
                <ModelPicker config={config} value={model} onChange={(value) => updateConfig("videoModel", value)} capability="video" fullWidth onMissingConfig={() => openConfigDialog(false)} testId="video-model-picker" />
            </label>
...
function ResultVideoCard({ video, onDownload, onSaveAsset }: { video: GeneratedVideo; onDownload: (video: GeneratedVideo) => void; onSaveAsset: (video: GeneratedVideo) => void }) {
    return (
        <div data-testid="video-result-card" className="overflow-hidden rounded-lg border border-stone-200 bg-background dark:border-stone-800">
...
function FailedVideoCard({ error, onRetry }: { error: string; onRetry: () => void }) {
    return (
        <div data-testid="video-failed-card" className="overflow-hidden rounded-lg border border-red-200 bg-red-50 dark:border-red-950 dark:bg-red-950/20">
