...
import { deleteAdminUser, fetchAdminUsers, saveAdminUser, type AdminUserPayload } from "@/services/api/users";
...
export function useAdminUsers() {
  ...
  const saveMutation = useMutation({
    mutationFn: (payload: AdminUserPayload) => saveAdminUser(token, payload),
    onSuccess: async (_, payload) => {
      await queryClient.invalidateQueries({ queryKey: ["admin", "users"] });
      message.success(payload.id ? "用户已保存" : "用户已新增");
    },
    onError: (error) => {
  ...
  const updateFilters = (next: Partial<{ keyword: string; page: number; pageSize: number }>) => {
    const state = { keyword, page, pageSize, ...next };
    if (next.keyword !== undefined || next.pageSize !== undefined) state.page = 1;
  ...
  return {
    ...
    saveUser: (payload: AdminUserPayload) => saveMutation.mutateAsync(payload),
