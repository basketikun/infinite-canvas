        ...
        >
            <div className="mb-2 flex items-center justify-between px-1">
                <span className="text-sm font-medium" style={{ color: theme.node.muted }}>
                    {t("canvas.connectionCreate.title")}
                </span>
            ...
            <div className="grid gap-1">
                <ConnectionCreateOption theme={theme} icon={<List className="size-5" />} title={t("canvas.connectionCreate.text")} description={t("canvas.connectionCreate.textDesc")} onClick={() => onCreate(CanvasNodeType.Text)} />
                <ConnectionCreateOption theme={theme} icon={<ImageIcon className="size-5" />} title={t("canvas.connectionCreate.image")} onClick={() => onCreate(CanvasNodeType.Image)} />
                <ConnectionCreateOption theme={theme} icon={<Video className="size-5" />} title={t("canvas.connectionCreate.video")} onClick={() => onCreate(CanvasNodeType.Video)} />
                <ConnectionCreateOption theme={theme} icon={<Music2 className="size-5" />} title={t("canvas.connectionCreate.audio")} onClick={() => onCreate(CanvasNodeType.Audio)} />
                <ConnectionCreateOption theme={theme} icon={<Settings2 className="size-5" />} title={t("canvas.connectionCreate.config")} description={t("canvas.connectionCreate.configDesc")} onClick={() => onCreate(CanvasNodeType.Config)} />
            </div>
        </div>
    );
}
...
export default function CanvasProjectPage() {
    ...
    const confirmStopGeneration = useCallback(
        (nodeId: string) => {
            modal.confirm({
                title: t("canvas.dialogs.stopGenerate.title"),
                content: t("canvas.dialogs.stopGenerate.content"),
                okText: t("canvas.dialogs.stopGenerate.ok"),
                cancelText: t("canvas.dialogs.stopGenerate.cancel"),
    ...
    useEffect(() => {
        ...
        void (async () => {
            const [restoredNodes, restoredSessions] = await Promise.all([hydrateCanvasImages(baseNodes), hydrateAssistantImages(baseSessions)]);
    ...
    const connectNodes = useCallback(
        (current: ConnectionHandle, targetNodeId: string) => {
            ...
            if (!connection) {
                message.warning(t("canvas.project.configNodesCannotConnect"));
                return;
            }
    ...
    const agentSnapshot = useMemo<CanvasAgentSnapshot>(
        () => ({ projectId, title: currentProject?.title || t("canvas.project.untitled"), nodes, connections, selectedNodeIds: Array.from(selectedNodeIds), viewport }),
        [connections, currentProject?.title, nodes, projectId, selectedNodeIds, t, viewport],
    );
    const applyAgentOps = useCallback(
        (ops?: CanvasAgentOp[]) => {
            const safeOps = Array.isArray(ops) ? ops.filter((op) => op?.type) : [];
            const before = { projectId, title: currentProject?.title || t("canvas.project.untitled"), nodes: nodesRef.current, connections: connectionsRef.current, selectedNodeIds: Array.from(selectedNodeIdsRef.current), viewport: viewportRef.current };
            ...
            return { ...next, projectId, title: currentProject?.title || t("canvas.project.untitled") };
        },
    ...
    const undoAgentOps = useCallback(() => {
        ...
        return { ...agentUndoSnapshot, projectId, title: currentProject?.title || t("canvas.project.untitled") };
    ...
    const createAndOpenProject = useCallback(() => {
        const id = createProject(t("canvas.project.newProjectName", { index: useCanvasStore.getState().projects.length + 1 }));
        navigate(`/canvas/${id}`);
    ...
    const pasteSystemClipboard = useCallback(async () => {
        ...
        if (imageItem) {
            ...
            message.success(t("canvas.project.addedImageFromClipboard"));
            return;
        }

        const text = await navigator.clipboard.readText();
        if (createTextNodeFromClipboard(text)) message.success(t("canvas.project.addedTextFromClipboard"));
    ...
    const saveNodeAsset = useCallback(
        async (node: CanvasNodeData) => {
            if (node.type === CanvasNodeType.Text) {
                ...
                addAsset({ kind: "text", title: node.metadata?.prompt?.slice(0, 24) || t("common.canvasText"), coverUrl: "", tags: [], source: "Canvas", data: { content }, metadata: { source: "canvas", nodeId: node.id } });
                message.success(t("common.addedToAssets"));
                return;
            }
            if (node.type === CanvasNodeType.Video) {
                if (!node.metadata?.content) return message.error(t("canvas.project.noVideoToSave"));
                addAsset({ kind: "video", title: node.metadata?.prompt?.slice(0, 24) || t("common.canvasVideo"), coverUrl: "", tags: [], source: "Canvas", data: { url: node.metadata.content, storageKey: node.metadata.storageKey, width: node.width, height: node.height, bytes: node.metadata.bytes || 0, mimeType: node.metadata.mimeType || "video/mp4" }, metadata: { source: "canvas", nodeId: node.id, prompt: node.metadata?.prompt } });
                message.success(t("common.addedToAssets"));
                return;
            }
            ...
            addAsset({
                kind: "image",
                title: node.metadata?.prompt?.slice(0, 24) || t("common.canvasImage"),
    ...
    const createImageReversePromptNodes = useCallback(
        (node: CanvasNodeData) => {
            if (node.type !== CanvasNodeType.Image || !node.metadata?.content) {
                message.warning(t("canvas.project.emptyImageCannotReverse"));
                return;
            }

            const reversePreset = t("canvas.project.reversePromptPreset");
            const gap = 96;
            ...
            const textNode = {
                ...createCanvasNode(
                    ...
                    { content: reversePreset, prompt: reversePreset, status: NODE_STATUS_SUCCESS, fontSize: 14 },
                ),
                title: t("canvas.project.reversePrompt"),
            };
            const configNode = {
                ...createCanvasNode(
                    ...
                    { x: textNode.position.x + textNode.width + gap + configSpec.width / 2, y: centerY },
                    {
                        ...
                        count: 1,
                        composerContent: t("canvas.project.reversePromptComposer", { imageId: node.id, textId: textNode.id }),
                    },
                ),
                title: t("canvas.project.reversePromptConfig"),
            };
    ...
    const splitImageNode = useCallback(
        async (node: CanvasNodeData, params: CanvasImageSplitParams) => {
            ...
            const childNodes = await Promise.all(
                pieces.map(async (piece) => {
                    ...
                    return {
                        id,
                        type: CanvasNodeType.Image,
                        title: t("canvas.project.splitPieceTitle", { title: node.title || t("common.image"), row: piece.row + 1, column: piece.column + 1 }),
            ...
            setSelectedConnectionId(null);
            setDialogNodeId(null);
            message.success(t("canvas.project.splitSuccess", { count: childNodes.length }));
        },
        [message],
    );
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            ...
            const prompt = t("canvas.project.maskEditPrompt", { prompt: userPrompt });
            const childId = nanoid();
            ...
            setNodes((prev) => [
                ...prev,
                {
                    id: childId,
                    type: CanvasNodeType.Image,
                    title: userPrompt.slice(0, 32) || t("canvas.project.maskEditResult"),
                    position: { x: node.position.x + node.width + 96, y: node.position.y },
                    width: node.width,
                    height: node.height,
    ...
    const generateAngleNode = useCallback(
        async (node: CanvasNodeData, params: CanvasImageAngleParams) => {
            ...
            try {
                ...
                if (isGenerationCanceled(error)) return;
                const errorDetails = error instanceof Error ? error.message : t("common.generateFailed");
                setNodes((prev) => prev.map((item) => (item.id === childId ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_ERROR, errorDetails } } : item)));
            } finally {
                finishGenerationRequest(childId, controller);
                setRunningNodeId(null);
    ...
    const startTitleEditing = useCallback(() => {
        setTitleDraft(currentProject?.title || t("canvas.project.untitled"));
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            const generationContext = await hydrateNodeGenerationContext(
                buildNodeGenerationContext(nodeId, nodesRef.current, connectionsRef.current, editingTextNode ? t("canvas.project.editTextPrompt", { source: sourceTextContent, prompt }) : prompt),
            );
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData) => {
            ...
            if (!prompt) {
                message.warning(t("canvas.project.promptNotFoundRetry"));
                return;
            }
            ...
            if (useReferenceImages && !retryReferenceImages) {
                message.error(t("canvas.project.referenceImageLost"));
                setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_ERROR, errorDetails: t("canvas.project.referenceImageLost") } } : item)));
                return;
            }
    ...
    const generateImageFromTextNode = useCallback(
        (node: CanvasNodeData) => {
            ...
            if (!prompt) {
                message.warning(t("canvas.project.emptyTextCannotGenerate"));
                return;
            }
    ...
    return (
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <section className="relative min-w-0 flex-1 overflow-hidden">
                <CanvasTopBar
                    title={currentProject?.title || t("canvas.project.untitled")}
                ...
                <Modal title={t("canvas.dialogs.superResolve.title")} open={Boolean(superResolveNode?.metadata?.content)} centered footer={null} onCancel={() => setSuperResolveNodeId(null)}>
                    <div className="py-8 text-center text-base font-medium">{t("canvas.dialogs.superResolve.notImplemented")}</div>
                </Modal>

                {angleNode?.metadata?.content ? <CanvasNodeAngleDialog dataUrl={angleNode.metadata.content} open={Boolean(angleNode)} onClose={() => setAngleNodeId(null)} onConfirm={(params) => void generateAngleNode(angleNode!, params)} /> : null}

                <Modal
                    title={t("canvas.dialogs.imagePreview.title")}
                ...
                <Modal
                    title={t("canvas.dialogs.clearCanvas.title")}
                ...
                >
                    <p className="text-sm opacity-60">{t("canvas.dialogs.clearCanvas.description")}</p>
                </Modal>
        ...
        <>
            <div className="pointer-events-none absolute left-0 right-0 top-0 z-50 flex h-16 items-center justify-between px-4">
                <div className="pointer-events-auto flex min-w-0 items-center gap-3">
                    <Dropdown
                        ...
                        menu={{
                            items: [
                                { key: "home", icon: <Home className="size-4" />, label: t("common.home"), onClick: onHome },
                                { key: "docs", icon: <BookOpen className="size-4" />, label: t("actions.docs"), onClick: () => window.open(DOCS_URL, "_blank", "noopener,noreferrer") },
                                { key: "projects", icon: <Images className="size-4" />, label: t("canvas.project.myCanvases"), onClick: onProjects },
                                { type: "divider" },
                                { key: "new", icon: <Plus className="size-4" />, label: t("canvas.project.newCanvas"), onClick: onCreateProject },
                                ...
                                { type: "divider" },
                                { key: "undo", disabled: !canUndo, icon: <Undo2 className="size-4" />, label: <MenuLabel text={t("common.undo")} shortcut="⌘ Z" />, onClick: onUndo },
                                { key: "redo", disabled: !canRedo, icon: <Redo2 className="size-4" />, label: <MenuLabel text={t("common.redo")} shortcut="⌘ ⇧ Z / ⌘ Y" />, onClick: onRedo },
                            ],
                        }}
                    >
                    ...
                    <div ref={titleRef} className="flex min-w-0 items-center gap-2">
                        {isTitleEditing ? (
                            ...
                            <button
                                ...
                                title={t("canvas.project.doubleClickRename")}
                            >
                                {title}
                            </button>
                        )}
            ...
            <Modal title={t("common.shortcuts")} open={shortcutsOpen} onCancel={() => setShortcutsOpen(false)} footer={null} centered>
                <div className="space-y-2 border-t pt-4 text-sm" style={{ borderColor: theme.node.stroke }}>
                    <Shortcut keys={[t("canvas.zoom.shortcutPan")]} value={t("canvas.zoom.shortcutPanValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutWheel")]} value={t("canvas.zoom.shortcutWheelValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutZoomSlider")]} value={t("canvas.zoom.shortcutZoomSliderValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutBoxSelect")]} value={t("canvas.zoom.shortcutBoxSelectValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutAddSelect")]} value={t("canvas.zoom.shortcutAddSelectValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutSelectAll")]} value={t("canvas.zoom.shortcutSelectAllValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutCopyPaste")]} value={t("canvas.zoom.shortcutCopyPasteValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutUndo")]} value={t("canvas.zoom.shortcutUndoValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutRedoShift")]} value={t("canvas.zoom.shortcutRedoShiftValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutRedoY")]} value={t("canvas.zoom.shortcutRedoYValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutDelete")]} value={t("canvas.zoom.shortcutDeleteValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutEsc")]} value={t("canvas.zoom.shortcutEscValue")} />
                    <Shortcut keys={[t("canvas.zoom.shortcutDropMedia")]} value={t("canvas.zoom.shortcutDropMediaValue")} />
                </div>
            </Modal>
        </>
    );
...
function CompactAgentStatus({ status, onClick }: { status: { connected: boolean; enabled: boolean; activity: string }; onClick: () => void }) {
    ...
    const colorTheme = useThemeStore((state) => state.theme);
    const theme = canvasThemes[colorTheme];
    const label = status.connected ? t("canvas.project.localCodexConnected") : status.enabled ? status.activity || t("canvas.project.localCodexConnecting") : t("canvas.project.localCodexStarting");
    ...
    return (
        <button
            ...
            onClick={onClick}
            title={t("canvas.project.openLocalCodexPanel")}
        >
...
function resetInterruptedGeneration(nodes: CanvasNodeData[]) {
    return nodes.map((node) => (node.metadata?.status === "loading" ? { ...node, metadata: { ...node.metadata, status: "error" as const, errorDetails: translate("canvas.project.refreshInterrupted") } } : node));
}

function isGenerationCanceled(error: unknown) {
    return error instanceof Error && (error.message === translate("errors.common.requestCancelled") || error.name === "AbortError");
}
...
function buildAngleLabel(params: CanvasImageAngleParams) {
    const horizontal = params.horizontalAngle === 0
        ? translate("canvas.project.angleFront")
        : params.horizontalAngle > 0
          ? translate("canvas.project.angleRotateRight", { degrees: params.horizontalAngle })
          : translate("canvas.project.angleRotateLeft", { degrees: Math.abs(params.horizontalAngle) });
    const pitch = params.pitchAngle === 0
        ? translate("canvas.project.angleLevel")
        : params.pitchAngle > 0
          ? translate("canvas.project.angleLookDown", { degrees: params.pitchAngle })
          : translate("canvas.project.angleLookUp", { degrees: Math.abs(params.pitchAngle) });
    return translate("canvas.project.angleLabel", {
        horizontal,
        pitch,
        distance: params.cameraDistance.toFixed(1),
        lens: params.wideAngle ? translate("canvas.project.angleWideLens") : translate("canvas.project.angleStandardLens"),
    });
}

function buildAnglePrompt(params: CanvasImageAngleParams) {
    return translate("canvas.project.anglePrompt", { label: buildAngleLabel(params) });
}
