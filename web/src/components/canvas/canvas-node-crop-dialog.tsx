...
export function CanvasNodeCropDialog({ dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (crop: CanvasImageCropRect) => void }) {
    ...
    return (
        <Modal title={t("canvas.dialogs.crop.title")} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={780} centered destroyOnHidden>
            <div className="space-y-4">
                <div className="flex justify-center">
                    <div ref={boxRef} className="relative inline-block max-w-full overflow-hidden rounded-lg bg-black select-none">
                        ...
                        <div className="absolute cursor-move border-2 border-white shadow-[0_0_0_1px_rgba(0,0,0,.3),0_0_28px_rgba(0,0,0,.28)]" style={cropStyle(crop)} onPointerDown={(event) => startDrag("move", event)}>
                            ...
                            {handles.map((handle) => (
                                <button key={handle} type="button" className="absolute size-3 rounded-full border border-black bg-white" style={handleStyle(handle)} onPointerDown={(event) => startDrag("resize", event, handle)} aria-label={t("canvas.dialogs.crop.adjustCropBox")} />
                            ))}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border px-3 py-2">
                    <div className="flex flex-wrap items-center gap-3 text-sm opacity-80">
                        <span>
                            {t("canvas.dialogs.crop.cropSize")} {cropSize ? `${cropSize.width} x ${cropSize.height}` : t("canvas.common.unknown")}
                        </span>
                        <span>
                            {t("canvas.dialogs.crop.ratio")} {cropSize ? formatRatio(cropSize.width, cropSize.height) : t("canvas.common.unknown")}
                        </span>
                        {image ? (
                            <span>
                                {t("canvas.dialogs.crop.original")} {image.width} x {image.height}
                            </span>
                        ) : null}
                    </div>
                    <Button icon={locked ? <Lock className="size-4" /> : <LockOpen className="size-4" />} onClick={() => setLocked((value) => !value)}>
                        {locked ? t("canvas.dialogs.crop.lockRatio") : t("canvas.dialogs.crop.freeRatio")}
                    </Button>
                </div>
