...
const changelogTypeKeys: Record<string, TranslationKey> = {
    新增: "version.changelogAdded",
    修复: "version.changelogFixed",
    调整: "version.changelogChanged",
    文档: "version.changelogDocs",
};
    ...
    return (
        <>
            <button
                ...
                title={t("actions.viewReleaseNotes")}
            >
            ...
            <Modal title={t("version.title")} open={open} width={680} centered footer={null} onCancel={() => setOpen(false)}>
                <div className="mb-5 grid grid-cols-2 gap-3">
                    <div className="rounded-lg border border-stone-200 p-3 dark:border-stone-800">
                        <div className="text-xs text-stone-500 dark:text-stone-400">{t("version.current")}</div>
                        <div className="mt-1 text-base font-semibold text-stone-950 dark:text-stone-100">{APP_VERSION}</div>
                    </div>
                    <div className="rounded-lg border border-stone-200 p-3 dark:border-stone-800">
                        <div className="flex items-center justify-between gap-3">
                            <div className="text-xs text-stone-500 dark:text-stone-400">{t("version.latest")}</div>
                            <button
                                type="button"
                                ...
                                <div>
                                    <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        <div className="flex min-w-0 items-center gap-1.5">
                                            {release.version === latestVersion ? <Tag color="green">{t("version.tagLatest")}</Tag> : null}
                                            {release.version === APP_VERSION ? <Tag>{t("version.tagCurrent")}</Tag> : null}
                                        </div>
                                    </div>
                                        ...
                                        {release.items.map((item, index) => (
                                            <div key={`${release.version}-${index}`} className="flex items-start gap-2 text-sm leading-6 text-stone-700 dark:text-stone-300">
                                                <Tag color={getTagColor(item.type)} className="m-0 mt-0.5 shrink-0 whitespace-nowrap">
                                                    {changelogTypeKeys[item.type] ? t(changelogTypeKeys[item.type]) : item.type}
                                                </Tag>
