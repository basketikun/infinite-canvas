export default {
    ...
    settingsPanels: {
        ...
        video: { title: "视频设置", quality: "清晰度", size: "尺寸", seconds: "秒数", resolution: "分辨率", ratio: "比例", duration: "时长", smart: "智能", output: "输出", generateAudio: "生成声音", watermark: "添加水印", seed: "种子", randomSeed: "每次随机生成", adaptive: "自适应", sizes: { landscape: "横屏", portrait: "竖屏", square: "方形", widescreen: "宽屏", tall: "长图", auto: "自动" }, ratios: { landscape: "横屏", portrait: "竖屏", square: "方形", standardLandscape: "标准横屏", standardPortrait: "标准竖屏", cinematic: "宽银幕", adaptive: "自适应" } },
    ...
    modelPlugin: {
        ...
        variables: { prompt: "用户输入的提示词（已拼接系统提示词）", images: "参考图，dataURL 数组（改图 / 图生视频时有值）", videoRefs: "参考视频，dataURL 或公开 URL 数组（视频生成连接了视频节点时有值）", audioRefs: "参考音频，dataURL 或公开 URL 数组（视频生成连接了音频节点时有值）", firstFrame: "FL2VA 首帧参考图，dataURL 数组", lastFrame: "FL2VA 尾帧参考图，dataURL 数组", messages: "对话消息数组，含系统消息", params: "生成参数：生图 {size,quality,count}、视频 {seconds,size,resolution,ratio,generateAudio,watermark}（内置 ComfyUI H3 模板另有 randomSeed、seed）、音频 {voice,format,speed,instructions}", workflow: "该模型上传的 API-format 工作流 JSON（已解析为对象），未上传时为 undefined", model: "模型名称（不含渠道前缀）", baseUrl: "渠道接口地址（原样，未拼 /v1）", apiKey: "渠道 API Key，请求头里自己带上", systemPrompt: "系统提示词原文", reasoningEffort: "文本推理强度；auto 表示由脚本决定是否传递", http: "便捷请求：http.post(path, body, {headers,params,responseType})、http.get(path, opts)、http.url(path)；默认带 Authorization: Bearer apiKey，可用 headers 覆盖；path 相对时按 baseUrl 拼 /v1", request: "原始请求 request({ method, url, headers, params, data, responseType })，不加任何默认头，鉴权头自己写；url 相对时按 baseUrl 拼接（不加 /v1）", poll: "轮询 poll(request, extract, {intervalMs,timeoutMs})，extract 返回真值即结束", sleep: "sleep(ms) 延时", signal: "取消信号，可透传给 http/request", onDelta: "onDelta(text) 推送流式文本（文本模型）", comfyui: "ComfyUI 原生工作流工具：规范化 Base URL、上传参考文件、提交和轮询 workflow、选择视频输出" },
    ...
    config: {
        ...
        scriptEditor: {
            ...
            workflowUpload: "上传工作流",
            workflowReplace: "替换工作流",
            workflowClear: "清除工作流",
            workflowReady: "工作流已上传（{{count}} 个节点）",
            workflowInvalid: "工作流文件不是有效的 JSON",
        },
        webdav: {
            title: "WebDAV 同步",
