export default {
    ...
    settingsPanels: {
        ...
        model: { select: "选择模型", assign: "请先在渠道里为{{capability}}指定模型", noMatch: "暂无匹配的{{capability}}模型", addFirst: "请先到配置里添加渠道和模型", noVideoProvider: "当前渠道没有提供视频模型", noImageInput: "不支持图片输入", capabilities: { image: "生图", video: "视频", text: "文本", audio: "音频" } },
    },
    ...
    modelPricing: { perMillionTokens: "/ 百万 tokens", perMillionImageTokens: "/ 百万图像 tokens", perMillionAudioTokens: "/ 百万音频 tokens", input: "输入", output: "输出", imageOutput: "图像输出", imageInput: "图像输入", audioInput: "音频输入" },
    apiErrors: { requestFailed: "请求失败", requestCanceled: "请求已取消", baseUrlRequired: "请先配置 Base URL", apiKeyRequired: "请先配置 API Key", authenticationFailed: "鉴权失败，请检查 API Key、套餐权限或模型权限", rateLimited: "请求被限流或额度不足，请稍后重试", notFound: "接口地址不存在（404），请检查 Base URL 和模型选择", badGateway: "网关错误（502），接口服务暂时不可用，请稍后重试", serviceBusy: "服务繁忙（503），请稍后重试", httpFailed: "请求失败（HTTP {{status}}），请检查 Base URL 和 API Key 是否正确", providerRefused: "渠道拒绝了本次请求（HTTP {{status}}）：{{message}}", htmlError: "服务返回了 HTML 错误页面（{{preview}}）", audioModelRequired: "请先配置音频模型", audioGenerationFailed: "音频生成失败", scriptNoAudio: "模型调用脚本没有返回音频", geminiAudioUnsupported: "Gemini 调用格式暂不支持音频生成，请使用 OpenAI 格式渠道", invalidImageSizeFormat: "图像尺寸格式不支持，请使用 auto、9:16 或 1024x1024", positiveImageRatio: "图像比例必须是正数，例如 9:16", imageRatioLimit: "图像宽高比不能超过 3:1，请调整尺寸", positiveImageDimensions: "图像尺寸必须是正整数，例如 1024x1024", imageDimensionStep: "图像尺寸的宽高必须是 16 的倍数，请调整尺寸", imageEdgeLimit: "图像尺寸最长边不能超过 3840px，请调整尺寸", imagePixelLimit: "图像总像素需在 655360 到 8294400 之间，请调整尺寸", unknownImageResponse: "接口返回了未知格式的数据（字段：{{fields}}），请检查模型或接口兼容性", noImageReturned: "接口没有返回图片，请检查提示词是否触发安全审核或模型是否支持该操作", geminiRejected: "Gemini 拒绝了本次请求：{{reason}}", geminiNoImage: "Gemini 接口没有返回图片", geminiMaskUnsupported: "Gemini 调用格式暂不支持蒙版编辑", maskModelUnsupported: "蒙版编辑暂不支持该模型，请使用其他渠道", noContent: "没有返回内容", modelReadFailed: "读取模型失败", videoTimeout: "{{provider}}视频生成超时，请稍后重试", pluginVideoExpired: "插件视频任务已失效，请重新生成", scriptNoVideo: "模型调用脚本没有返回视频", noPlayableVideo: "视频接口没有返回可播放的视频", noVideoTaskId: "视频接口没有返回任务 ID", videoTaskCreateFailed: "视频任务创建失败", videoGenerationFailed: "视频生成失败", videoTaskQueryFailed: "视频任务查询失败", videoPromptRequired: "请输入视频提示词，或连接参考图片/视频/音频", referenceImageReadFailed: "参考图读取失败，请换一张图片或重新上传", invalidReferenceVideo: "参考视频必须是公网 URL、资产 ID，或本地已保存的视频", invalidReferenceAudio: "参考音频必须是公网 URL、资产 ID，或本地已保存的音频", videoModelRequired: "请先配置视频模型", geminiVideoUnsupported: "Gemini 调用格式暂不支持视频生成，请使用 OpenAI 格式渠道", noVideoTask: "接口没有返回视频任务", videoDownloadFailed: "视频下载失败", localAssetReadFailed: "读取本地资产失败" },
    prompts: {
        title: "提示词中心",
        library: "提示词库",
    ...
    workbench: {
        ...
        noVideoModels: "当前渠道没有视频模型。fal.ai、Replicate、OpenAI（Sora）和 Google（Veo）提供视频生成。",
        addVideoProvider: "添加视频渠道",
    ...
    config: {
        ...
        channels: {
            ...
            missingUrl: "未填写接口地址",
            keepOne: "至少保留一个渠道",
            notConnected: "未填写 API Key，该渠道模型不会出现在工作台",
            defaultName: "默认渠道",
            newName: "新渠道",
        ...
        providers: {
            ...
            hints: {
                fal: "Kling、Veo 3、MiniMax、Luma 视频 + FLUX 生图（队列 API）",
                gemini: "Veo 3.1 视频、Nano Banana 生图、Gemini 文本",
                openai: "Sora 2 视频、GPT Image、TTS",
                xai: "Grok Imagine 视频 + 生图",
                replicate: "Kling、Seedance、Wan、MiniMax 视频",
                openrouter: "400+ 个模型，一个 Key 全搞定",
            },
        },
        preferences: {
            interface: "界面偏好",
        ...
        channelEditor: {
            ...
            baseUrl: "接口地址",
            models: "渠道模型",
            modelDescription: "已导入 {{count}} 个模型；在工作台里选择要用哪个。",
            selectModels: "选择模型",
            connect: "连接",
            noAuth: "该渠道无需 API Key",
            noAuthHint: "适用于本地或自建网关：按 IP 鉴权，或完全不鉴权。",
            refresh: "刷新模型列表",
