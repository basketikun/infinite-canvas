export default {
    ...
    settingsPanels: {
        ...
        video: { title: "Video settings", quality: "Quality", size: "Size", seconds: "Seconds", resolution: "Resolution", ratio: "Aspect ratio", duration: "Duration", smart: "Smart", output: "Output", generateAudio: "Generate audio", watermark: "Add watermark", seed: "Seed", randomSeed: "Randomize each run", adaptive: "Adaptive", sizes: { landscape: "Landscape", portrait: "Portrait", square: "Square", widescreen: "Widescreen", tall: "Tall", auto: "Auto" }, ratios: { landscape: "Landscape", portrait: "Portrait", square: "Square", standardLandscape: "Standard landscape", standardPortrait: "Standard portrait", cinematic: "Cinematic", adaptive: "Adaptive" } },
    ...
    modelPlugin: {
        ...
        variables: { prompt: "User prompt with the system prompt already included", images: "Reference images as a data URL array, available for image editing and image-to-video", videoRefs: "Reference videos as data URL or public URL arrays, available when video nodes are connected", audioRefs: "Reference audio as data URL or public URL arrays, available when audio nodes are connected", firstFrame: "FL2VA first-frame reference images as a data URL array", lastFrame: "FL2VA last-frame reference images as a data URL array", messages: "Conversation message array including the system message", params: "Generation parameters: image {size,quality,count}, video {seconds,size,resolution,ratio,generateAudio,watermark} (built-in ComfyUI H3 templates also receive randomSeed and seed), audio {voice,format,speed,instructions}", workflow: "API-format workflow JSON uploaded for this model as a parsed object; undefined when absent", model: "Model name without the provider prefix", baseUrl: "Provider endpoint as entered, without appending /v1", apiKey: "Provider API key; add it to request headers yourself", systemPrompt: "Original system prompt", reasoningEffort: "Text reasoning effort; auto lets the script decide whether to send it", http: "Convenience client: http.post(path, body, {headers,params,responseType}), http.get(path, opts), and http.url(path). Authorization: Bearer apiKey is included by default and can be overridden. Relative paths append /v1 to baseUrl.", request: "Raw request({ method, url, headers, params, data, responseType }) with no default headers. Add authentication yourself. Relative URLs are joined to baseUrl without /v1.", poll: "poll(request, extract, {intervalMs,timeoutMs}) continues until extract returns a truthy value", sleep: "Delay with sleep(ms)", signal: "Cancellation signal that can be passed to http/request", onDelta: "Push streaming text with onDelta(text) for text models", comfyui: "Native ComfyUI workflow tools for Base URL normalization, reference upload, workflow submission and polling, and video-output selection" },
    ...
    config: {
        ...
        scriptEditor: {
            ...
            workflowUpload: "Upload workflow",
            workflowReplace: "Replace workflow",
            workflowClear: "Clear workflow",
            workflowReady: "Workflow uploaded ({{count}} nodes)",
            workflowInvalid: "The workflow file is not valid JSON",
        },
        webdav: {
            title: "WebDAV sync",
