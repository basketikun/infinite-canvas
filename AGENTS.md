...
- 本地开发模式下画布项目、“我的素材”、媒体、生成记录和配置通过本机共享持久化服务保存到 `data/infinite-canvas`，这不是云同步；静态部署仍按浏览器本地存储处理。
- 本地开发模式的共享数据文件包含 AI API Key，前端仍直接请求 OpenAI 兼容接口；涉及安全说明时要提醒用户不要复制或提交 `data/infinite-canvas`。
