...
+ [新增] ComfyUI 视频模型插件支持参考图、参考视频、参考音频及 FL2VA 首尾帧角色化输入。
+ [新增] 模型脚本支持按模型上传 API-format workflow，并提供共享的 ComfyUI 上传、提交、轮询和视频输出解析工具。
+ [优化] 视频生成配置仅在模型脚本声明支持时显示首帧、尾帧端口，其他模型继续使用普通参考图输入。
+ [调整] H3 比例/分辨率与随机种子界面仅对内置 Ref2VA、FL2VA 模板生效，其他视频模型恢复官方尺寸设置。
+ [新增] H3 随机生成会记录实际 seed，关闭“每次随机生成”后可锁定该 seed 调高质量重做。
+ [修复] H3 分辨率显示与模板参数改为复刻 ComfyUI `ResolutionSelector` 的 1024² 百万像素换算并读取 workflow 的倍数取整，当前 32 倍数下 720p 横屏正确显示 `1280×736`。
+ [修复] ComfyUI 原生请求不再错误拼接 `/v1`，安全重试上传与轮询但不重复提交生成，并优先返回真实视频输出。
+ [修复] Ref2VA 保留画布参考图顺序，FL2VA 独立按首尾帧角色注入；本地 H3 模板不再强制填写未使用的 API Key。
+ [修复] 无论从素材节点还是首尾帧端口开始拖线，FL2VA 首帧与尾帧角色都能正确保留。
