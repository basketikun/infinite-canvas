...
+ [新增] 本地 Agent 对话栏支持读取 Codex 可用模型并选择模型与推理强度，切换选择时保留当前对话。
+ [新增] 本地 Agent 新增 `assets_add_file` 工具，可把工作区本地图片直接加入「我的素材」。
+ [修复] 画布在局域网 HTTP 页面复制图片时可通过已连接的 Windows 本机 Agent 写入系统剪贴板，避免浏览器兼容模式复制失效。
+ [新增] 本地 Agent 历史页支持选择 Codex 最近使用过的工作空间，或输入已存在的本机绝对目录进行切换。
