...
+ [新增] 本地开发模式增加本机共享持久化，让 Codex 内置浏览器和外部浏览器共用画布、素材、媒体、生成记录与配置数据。
+ [修复] Director「导出当前帧」改为将 PNG 直接回写为无限画布图片节点，不再依赖本地导出服务。
+ [调整] Blockout 素材库分类默认折叠，点击分类标题后再展开具体素材。
+ [修复] Embedded Blockout 接通浏览器文件选择与 Director Bridge 文件存储，支持导入 3D 模型、3D 扫描和参考视频并在刷新后读取。
+ [修复] Director 节点悬停层恢复「进入 3D 导演台」点击打开 Workspace，同时不影响节点拖动。
+ [修复] Director 节点内容不再拦截画布宿主的拖动事件，恢复节点的选中、拖动和双击进入逻辑。
+ [优化] Director 节点改为以 3D 镜头预览为主，精简状态信息并将进入导演台操作收进悬停态。
+ [优化] Blockout Embedded 工作台新增 Canvas Director Skin，统一顶部栏、面板、控件和时间轴的低视觉重量样式。
+ [优化] Blockout 工作台顶部栏重组项目身份、模式切换和操作区，增强保存状态提示与窄宽度下的可读性。
+ [优化] Blockout 顶部栏进一步采用三列居中布局，重做 Director 项目标识，减少视觉噪音并保持左右操作区独立对齐。
+ [调整] Blockout 工作台所有下拉控件统一为可主题化的自定义菜单，支持分组、键盘操作和窗口边界自适应。
...
+ [优化] 提示词助理浮动面板统一生图工作台的浅色卡片、细边框和紧凑间距，融入现有页面视觉。
+ [修复] 生图工作台提示词助理兼容仅提供 Chat Completions 接口的文本模型，避免把接口错误直接显示成对话内容。
+ [调整] 提示词助理将应用操作固定在每条 AI 回复气泡底部，点击时只回填当前回复中的最终提示词。
+ [优化] 提示词助理输入框改为与 Codex 一致的上下分区布局，文本模型移至底部操作栏。
+ [新增] 网页右上角增加退出按钮，本地运行时可停止 Infinite Canvas 服务并退出当前页面。
+ [修复] Eagle 视频文件接口支持 HTTP Range 分段读取，视频进度条可以拖动跳转。
+ [修复] 视频资产详情跳过无意义的顶部封面兜底区域，不再显示“暂无封面”。
+ [修复] 视频资产详情不再同时显示缩略图和视频播放器，避免重复展示媒体内容。
+ [修复] 资产详情抽屉按类型显示文本内容、图片内容和视频预览，避免视频播放器误标为文本内容。
+ [修复] Eagle 媒体 Bridge 支持 HEIC/HEIF 正确 MIME 类型，并确保本地服务以可访问 Eagle 素材库的权限运行。
+ [修复] Eagle 素材文件无权限读取时不再导致本地 Bridge/Vite 服务崩溃。
+ [修复] Windows 一键启动脚本改为记录 npm 启动输出，并在启动失败时显示具体原因。
+ [新增] 提示词来源新增 Freestylefly GPT Image 2 内置来源。
+ [调整] 资产目录统一读取项目本地资产与 Eagle 资产，资产页、画布资产面板、资产选择器和 Agent 资产列表均显示 Eagle 文件夹树及数量。
+ [新增] 资产保存入口支持选择项目本地、Eagle 未分类或任意 Eagle 子文件夹；图片、视频和文本工作流复用同一套读写逻辑，并支持将文本作为 Eagle 素材保存。
+ [优化] 生图结果卡片、画布资产操作栏和资产页顶部工具栏统一按钮层级、响应式换行与水平对齐，减少重复弹窗和操作错位。
+ [新增] 增加 Windows 一键启动脚本，方便本地以应用方式启动 Infinite Canvas。
