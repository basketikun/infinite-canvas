...
+ [新增] 视频反推提示词完整落地：视频节点「反推提示词」+ 文本配置节点连接视频节点均支持；Gemini 渠道原生视频直传（含音频/时序），超 20MB 或读取失败自动降级为抽帧，其余后端统一抽帧兜底。
+ [新增] 视频节点新增「反推提示词」：浏览器按时长自适应抽帧（5~9 帧）后复用文本能力反推视频提示词，后端无关（Gemini 等原生视频直传留待 Phase 2，见视频反推方案）。
