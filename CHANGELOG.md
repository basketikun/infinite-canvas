...
+ [修复] ComfyUI H3 全能参考的参考视频槽位接入方式：`ref_videos` 期望 24fps 视频帧（IMAGE）而非 VIDEO 类型，改为经 `GetVideoComponents.images` 接入；`ref_video_audios` 接其 `audio` 输出；参考裁剪升级为可达性分析，保留 LoadVideo → GetVideoComponents 引用链。
+ [修复] ComfyUI H3 全能参考模板对 Cloudflare 隧道的间歇断连自动重试（上传 / 提交 / 轮询最多重试 3 次），避免生成时偶发 "Network Error"。
+ [修复] ComfyUI H3 全能参考模板参考槽裁剪改为使用上传时记录的参考数量，避免注入循环 shift 清空数组后把全部参考槽误裁（此前参考图/视频/音频无法真正注入工作流）。
+ [新增] 模型插件脚本新增 `videoRefs` / `audioRefs` 变量，视频模型可读取连接的参考视频与参考音频；内置「ComfyUI H3 全能参考」视频模板（最多 9 图 + 3 视频 + 3 音频共 12 个参考文件）。
