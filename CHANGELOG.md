...
+ [新增] 提供 Windows x64 本地桌面版，打包网页、Node.js 与 Canvas Agent，启动应用时自动拉起并连接 Agent。
+ [新增] 桌面版支持多个 OpenAI 兼容渠道、独立 Agent 文本模型，以及由用户显式指定模型的文本、图片、视频和音频能力。
+ [安全] 桌面版将渠道 API Key 与 WebDAV 密码迁移到 Windows 凭据管理器，并对 Agent 日志中的连接 Token 脱敏。
+ [优化] 桌面版可直接请求 HTTP 或 HTTPS 模型接口；非本机 HTTP 渠道会显示明文传输风险提示。
+ [优化] Windows 桌面版延迟到首次打开 Agent 面板后再启动 Codex 与 MCP，避免应用启动时后台预热造成画布卡顿和按钮无响应。
+ [新增] GitHub Windows 流水线负责构建 NSIS 安装包、签名更新文件并发布到 R2，客户端后台检查更新并在用户确认后安装。
