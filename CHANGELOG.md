...
+ [修复] 模型能力识别改为按分隔符整词匹配，修复 `inkling` 等文本模型因包含 `kling` 子串被误判为视频模型的问题。
+ [修复] 加载已保存配置时自动修正与关键词不符的模型能力标注，并清空能力不匹配的默认模型（如被误设为默认视频模型的文本模型）。
+ [新增] 新增渠道支持服务商预设（fal.ai、Google Gemini、OpenAI、xAI、Replicate、OpenRouter）：一键创建渠道并预置模型能力与调用脚本，OpenAI 兼容服务商可自动拉取全部模型，OpenRouter 无需 Key 即可在线拉取完整模型目录。
+ [调整] 渠道编辑中的「协议」字段更名为「接口格式」，并在选择预设服务商时自动填充。
+ [修复] OpenRouter 图生图不再丢失参考图：调用脚本改为按官方格式传入 `input_references`，`n>1` 被拒后的逐张重试同样携带参考图。此前参考图被静默丢弃，图生图实际退化为纯文生图。
+ [修复] 模型能力改为优先采用服务商声明的模态（OpenRouter `architecture.output_modalities`），仅在服务商未提供时才回退到关键词猜测，文本/生图模型不再互相错位。
+ [修复] 加载配置时不再覆盖用户在渠道编辑器中手动指定的能力：仅重新判定「猜测」来源的条目，服务商声明与用户设置的能力保持不变。
+ [修复] 关键词匹配修复带连字符的词条（`gpt-image`、`dall-e`、`stable-diffusion`），此前这些词条因整词匹配永远无法命中。
+ [修复] 当前渠道没有对应能力的模型时，不再回退到并不存在的内置默认模型（如 `grok-imagine-video`），避免发出注定失败的请求；创作台也不再跨能力回退（视频创作台不会再显示生图模型）。
+ [新增] 模型选择器、模型选择弹窗与渠道编辑器显示服务商公布的价格（OpenRouter 全量目录、xAI），单位与服务商一致（每百万 tokens）；未公布价格的服务商留空而不是显示 $0.00。
