...
export const toolInputSchemas = {
  ...
  assets_add: z.object({
    ...
    target: z.enum(["local", "eagle"]).optional(),
    eagleFolderId: z.string().optional(),
  }),
...
export const toolDescriptions: Record<ToolName, string> = {
  ...
  generation_get_status:
    "查询当前活动网页的生成任务状态。默认返回画布、生图工作台和视频工作台最近任务；可用 scope 过滤来源，用 taskId 查询工作台任务，用 nodeIds 查询画布节点。",
  workbench_image_get_config:
    "读取生图工作台的当前参数和可选项（可用模型、质量、尺寸/宽高比、张数范围），在调用 workbench_image_generate 前先了解可选值。",
  workbench_image_generate:
    "在生图工作台填入提示词并按需设置 model、quality、size（如 1:1 或 1024x1024）、count，run 默认 true 会自动点击生成按钮。会自动跳转到生图工作台。生成为异步过程，提交后返回 taskId，可用 generation_get_status 查询状态。",
  workbench_video_get_config:
  ...
  prompts_search:
    "搜索提示词库（第三方提示词合集），支持 keyword、category、tags 过滤和 page/pageSize 分页，返回标题、提示词、分类、标签、封面等。",
  assets_list:
    "列出用户「我的素材」中的项目本地与 Eagle 资产，支持 kind（text/image/video）过滤、keyword 搜索和 page/pageSize 分页。为控制体积不返回图片/视频原始 data，仅返回封面与元信息。",
  assets_add:
    "向「我的素材」新增素材。kind=text 时用 content 传文本内容；kind=image 时用 imageUrl 传图片地址或 dataURL。默认保存到项目本地；target=eagle 时写入 Eagle，eagleFolderId 可指定 Eagle 文件夹。可附带 title、tags、source、note。",
};
