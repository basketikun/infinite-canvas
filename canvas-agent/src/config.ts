...
export const CONFIG_DIR = process.env.CANVAS_AGENT_CONFIG_DIR?.trim() || path.join(os.homedir(), ".infinite-canvas");
...
export function loadConfig(create = false): CanvasAgentConfig {
    try {
        ...
        const token = process.env.CANVAS_AGENT_TOKEN?.trim();
        return token ? { ...config, token } : config;
    } catch {
        const configuredPort = process.env.PORT === undefined ? DEFAULT_PORT : Number(process.env.PORT);
        const port = Number.isInteger(configuredPort) && configuredPort >= 0 ? configuredPort : DEFAULT_PORT;
        const config = { url: `http://127.0.0.1:${port}`, token: process.env.CANVAS_AGENT_TOKEN?.trim() || crypto.randomBytes(18).toString("hex") };
...
export function saveConfig(config: CanvasAgentConfig) {
    writeConfigFile(CONFIG_DIR, CONFIG_FILE, configForPersistence(config));
}

/** 桌面托管模式使用每次启动生成的令牌，不把该令牌写入普通配置文件。 */
export function configForPersistence(config: CanvasAgentConfig, ephemeralToken = process.env.CANVAS_AGENT_EPHEMERAL_TOKEN === "1") {
    return ephemeralToken ? { ...config, token: "" } : config;
}
