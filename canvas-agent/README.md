...
仓库内提供了 Codex app 插件：`plugins/infinite-canvas`。在 Codex app 中添加本仓库的 marketplace 后，可以安装 `Cozy Tv` 插件；插件会注册同一个 `infinite-canvas` MCP，并带上画布操作说明。
...
使用时可以直接在 Codex 里说“打开 Cozy Tv”，插件会优先启动本地画布和本地 Agent，读取 Local URL 和 Connect token，然后直接打开画布网页地址新建并连接画布。如果自动连接失败，再检查本地画布服务和 Canvas Agent 是否都已启动。
...
```bash
codex mcp add infinite-canvas -- node /path/to/cozy-tv/canvas-agent/dist/index.js mcp
```
...
```bash
claude mcp add --scope user --transport stdio infinite-canvas -- node /path/to/cozy-tv/canvas-agent/dist/index.js mcp
```
